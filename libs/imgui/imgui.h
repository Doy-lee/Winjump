...
// same value as passed to your io.RenderDrawListsFn() function. valid after
// Render() and until the next call to NewFrame()
IMGUI_API ImDrawData *GetDrawData();

// start a new ImGui frame, you can submit any command from this point until
// NewFrame()/Render().
IMGUI_API void NewFrame();

// ends the ImGui frame, finalize rendering data, then call your
// io.RenderDrawListsFn() function if set.
...
// style editor block. you can pass in a reference ImGuiStyle structure to
...
// 'bool* p_open' creates a widget on the upper-right to close the window (which
...
// For regular windows, 'size_on_first_use' only applies to the first time EVER
// the window is created and probably not what you want! might obsolete this API
// eventually.
IMGUI_API bool Begin(const char *name, bool *p_open, const ImVec2 &size_on_first_use,
                     float bg_alpha = -1.0f, ImGuiWindowFlags flags = 0);

// finish appending to current window, pop it off the window stack.
IMGUI_API void End();

// begin a scrolling region. size==0.0f: use remaining window size, size<0.0f:
// use remaining window size minus abs(size). size>0.0f: fixed size. each axis
// can use a different mode, e.g. ImVec2(0,400).
...
// per-window font scale. Adjust IO.FontGlobalScale if you want to scale all
// windows
...
// set next window content size (enforce the range of scrollbars). set axis to
// 0.0f to leave it automatic. call before Begin()
IMGUI_API void SetNextWindowContentSize(const ImVec2 &size);

// set next window content width (enforce the range of horizontal scrollbar).
// call before Begin()
...
// (not recommended) set current window to be focused / front-most. prefer using
// SetNextWindowFocus().
...
// adjust scrolling amount to make given position valid. use GetCursorPos() or
...
// components of a multiple component widget. Use negative 'offset' to access
// previous widgets.
IMGUI_API void SetKeyboardFocusHere(int offset = 0);

// replace tree state storage with our own (if you want to manipulate it
...
// width of items for the common item+label case, pixels. 0.0f = default to ~2/3
// of windows width, >0.0f: width in pixels, <0.0f align xx pixels to the right
// of window (so -1.0f always align width to the right side)
...
// word-wrapping for Text*() commands. < 0.0f: no wrapping; 0.0f: wrap to end of
// window (or column); > 0.0f: wrap at 'wrap_pos_x' position in window local
// space
...
// manner (uses io.KeyRepeatDelay/io.KeyRepeatRate for now). Note that you can
...
// move content position back to the left, by style.IndentSpacing or indent_w if
// >0
...
// cursor position in absolute screen coordinates [0..io.DisplaySize] (useful to
...
// call once if the first item on the line is a Text() item and you want to
// vertically lower it to match subsequent (bigger) widgets
...
// == GetWindowFontSize() + GetStyle().FramePadding.y*2 +
...
// setup number of columns. use an identifier to distinguish multiple column
// sets. close with Columns(1).
...
// inclusive. column 0 is usually 0.0f and not resizable unless you call this
...
// column width (== GetColumnOffset(GetColumnIndex()+1) -
...
// calculate unique ID (hash of whole ID stack + given parameter). useful if you
// want to query into ImGuiStorage yourself. otherwise rarely needed
...
// doesn't require null terminated string if 'text_end' is specified. no copy
...
// draw a small circle and keep the cursor on the same line. advance cursor x
...
// Hint: 'float col[3]' function argument is same as 'float* col'. You can pass
// address of first element out of a contiguous set, e.g. &myvector.x
IMGUI_API bool ColorEdit3(const char *label, float col[3]);

// "
IMGUI_API bool ColorEdit4(const char *label, float col[4], bool show_alpha = true);

// FIXME-OBSOLETE: This is inconsistent with most of the API and will be
// obsoleted/replaced.
...
// adjust display_format to decorate the value with a prefix or a suffix. Use
...
IMGUI_API bool VSliderFloat(const char *label, const ImVec2 &size, float *v, float v_min,
                            float v_max, const char *display_format = "%.3f", float power = 1.0f);
IMGUI_API bool VSliderInt(const char *label, const ImVec2 &size, int *v, int v_min, int v_max,
                          const char *display_format = "%.0f");

// Widgets: Trees

// if returning 'true' the node is open and the tree id is pushed into the id
// stack. user is responsible for calling TreePop().
IMGUI_API bool TreeNode(const char *label);

// read the FAQ about why and how to use ID. to align arbitrary text at the same
...
// you can call Push/Pop yourself for layout purpose
...
// when 'p_open' isn't NULL, display an additional small close button on upper
// right of the header
IMGUI_API bool CollapsingHeader(const char *label, bool *p_open, ImGuiTreeNodeFlags flags = 0);

// Widgets: Selectable / Lists

// size.x==0.0: use remaining width, size.x>0.0: specify width. size.y==0.0: use
...
// make sure to call ListBoxFooter() afterwards.
...
// set tooltip under mouse-cursor, typically use with ImGui::IsHovered(). last
// call wins
...
// create and append to a full screen menu-bar. only call EndMainMenuBar() if
// this returns true!
IMGUI_API bool BeginMainMenuBar();
IMGUI_API void EndMainMenuBar();

// append to menu-bar of current window (requires ImGuiWindowFlags_MenuBar flag
// set). only call EndMenuBar() if this returns true!
...
// mark popup as open. popups are closed when user click outside, or activate a
// pressable item, or CloseCurrentPopup() is called within a
// BeginPopup()/EndPopup() block. popup identifiers are relative to the current
// ID-stack (so OpenPopup and BeginPopup needs to be at the same level).
IMGUI_API void OpenPopup(const char *str_id);

// return true if the popup is open, and you can start outputting to it. only
// call EndPopup() if BeginPopup() returned true!
...
// helper to open and begin popup when clicked on last item. read comments in
// .cpp!
IMGUI_API bool BeginPopupContextItem(const char *str_id, int mouse_button = 1);

// helper to open and begin popup when clicked on current window.
IMGUI_API bool BeginPopupContextWindow(bool also_over_items = true, const char *str_id = NULL,
                                       int mouse_button = 1);
...
// close the popup we have begin-ed into. clicking on a MenuItem or Selectable
...
// was the last item hovered by mouse? even if another item is active or window
// is blocked by popup while we are hovering this
IMGUI_API bool IsItemHoveredRect();

// was the last item active? (e.g. button being held, text field being edited-
...
// is current root window focused (root = top-most parent of a child, otherwise
// self)
...
// is current root window or any of its child (including current window) hovered
// and hoverable (not blocked by a popup)
...
// utility to find the closest point the last item bounding rectangle edge.
// useful to visually link items
...
// calculate coarse clipping for large list of evenly sized items. Prefer using
// the ImGuiListClipper higher-level helper if you can.
...
// key_index into the keys_down[] array, imgui doesn't know the semantic of each
// entry, uses your own indices!
...
// is mouse hovering current window ("window" in API names always refer to
...
// clipping settings. disregarding of consideration of focus/window
// ordering/blocked by a popup.
...
// dragging amount since clicking. if lock_threshold < -1.0f uses
...
// get desired cursor type, reset in ImGui::NewFrame(), this updated during the
// frame. valid before Render(). If you use software rendering by setting
...
// manually override io.WantCaptureKeyboard flag next frame (said flag is
// entirely left for your application handle). e.g. force capture keyboard when
// your widget is being hovered.
IMGUI_API void CaptureKeyboardFromApp(bool capture = true);

// manually override io.WantCaptureMouse flag next frame (said flag is entirely
// left for your application handle).
